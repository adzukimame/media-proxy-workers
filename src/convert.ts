export function convertToStatic(file: { mime: string; ext: string | null; filename: string; buffer: Uint8Array }):
{
  mime: string;
  ext: string | null;
  filename: string;
...
function convertApngToStatic(src: Uint8Array): Uint8Array {
  const srcLength = src.length;
  const dst = new Uint8Array(new ArrayBuffer(src.byteLength));
...
function convertGifToStatic(src: Uint8Array): Uint8Array {
  const srcLength = src.length;
  const dst = new Uint8Array(new ArrayBuffer(src.byteLength));

  // if GIF header (GIF89a) and Logical Screen Descriptor are truncated
  if (12 >= srcLength) return src;
  ...
  while (srcIdx < srcLength) {
    ...
    if (src[srcIdx] === 0x21) {
      ...
      if (src[srcIdx + 1] === 0xff) {
        ...
        if (!copiedApplicationExtension && src[srcIdx + 3] === 0x4e && src[srcIdx + 4] === 0x45 && src[srcIdx + 5] === 0x54 && src[srcIdx + 6] === 0x53 && src[srcIdx + 7] === 0x43 && src[srcIdx + 8] === 0x41 && src[srcIdx + 9] === 0x50 && src[srcIdx + 10] === 0x45 && src[srcIdx + 11] === 0x32 && src[srcIdx + 12] === 0x2e && src[srcIdx + 13] === 0x30) {
          ...
          if (srcIdx + 18 >= srcLength) return src;
      ...
      // Other extension blocks or duplicate extension blocks
        ...
        while (true) {
          ...
          if (srcIdx >= srcLength) return src;
    ...
    // Image Block
      ...
      if (srcIdx + 9 >= srcLength) return src;
      ...
      if (localColorTableFlag) {
        ...
        if (srcIdx + localColorTableSize * 3 - 1 >= srcLength) return src;
      ...
      while (true) {
        ...
        if (srcIdx + blockSize >= srcLength) return src;
